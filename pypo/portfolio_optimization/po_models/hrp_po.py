"""
The ``hrp_po`` (hierarchical risk-parity portfolio optimization) module seeks to implement one of the recent advances in
portfolio optimization – the application of hierarchical clustering models in allocation.

All of the hierarchical classes have a similar API to ``mv_po``, though since
...
- ``HRPPO`` implements the Hierarchical Risk Parity (HRP) portfolio. Code reproduced with
...
from pypo.portfolio_optimization.po_optimizers import base_optimizer
...
class HRPPO(base_optimizer.BaseOptimizer):
    ...
    def _raw_hrp_allocation(cov, ordered_tickers):
        ...
        while len(cluster_items) > 0:
            ...
            for i in range(0, len(cluster_items), 2):
                ...
                first_variance = HRPPO._get_cluster_var(cov, first_cluster)
                second_variance = HRPPO._get_cluster_var(cov, second_cluster)
    ...
    def optimize(self, linkage_method="single"):
        ...
        sort_ix = HRPPO._get_quasi_diag(self.clusters)
        ordered_tickers = corr.index[sort_ix].tolist()
        hrp = HRPPO._raw_hrp_allocation(cov, ordered_tickers)
