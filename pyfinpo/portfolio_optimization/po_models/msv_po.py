...
from pypo.portfolio_optimization.po_models.mv_po import MeanVariancePO
from pypo.portfolio_optimization import po_objective_functions


class MeanSemivariancePO(MeanVariancePO):
    """
    ...
    def max_quadratic_utility(self, risk_aversion=1, market_neutral=False):
        ...
        if update_existing_parameter:
            ...
            p = cp.Variable(self._T, nonneg=True)
            n = cp.Variable(self._T, nonneg=True)
            mu = po_objective_functions.portfolio_return(self._w, self.expected_returns)
            mu /= self.frequency
    ...
    def portfolio_performance(self, verbose=False, risk_free_rate=0.02):
        ...
        """
        mu = po_objective_functions.portfolio_return(
